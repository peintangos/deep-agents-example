...
- [x] spec-009: `scripts/run-audit.ts` の CLI に `--target <owner/repo>` オプションを追加し、任意の GitHub リポジトリを監査対象として指定可能にする — `src/cli.ts` に pure 関数 `parseTargetArg` (形式チェック) / `buildAuditPrompt` (対象 identity のみの薄いユーザープロンプト) を追加し、意味的バリデーションは spec-008 の `validateGithubRepoArgs` を再利用。`--invoke` との排他を runCli の先頭で明示的に弾き、invoker 未設定 / invoker throw / 形式不正 / owner trailing hyphen / repo 先頭ドット等をすべて stderr + exitCode=1 で返す。`tests/cli.test.ts` に 14 ケース追加 (runCli --target 8 + parseTargetArg 4 + buildAuditPrompt 2)、合計 333 tests 全通過
