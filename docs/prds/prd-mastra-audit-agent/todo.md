...
- [ ] spec-008: `src/middleware/rate-limit.ts` で GitHub API レート制限対応 middleware を実装する (閾値ベースで最小待機時間を挿入)
- [ ] spec-008: `src/middleware/validate.ts` でツール引数バリデーション middleware を実装する (不正なリポジトリ URL 等を handler 呼び出し前に弾く)
- [ ] spec-008: `createAuditAgent()` の `middleware` オプションに 3 つの middleware を配線し、wrap 順序を統合テストで確認する (`middleware?` を `CreateAuditAgentOptions` に DI で追加)
