...
- [x] `createDeepAgent()` の `middleware` オプションに登録 — `CreateAuditAgentOptions.middleware` を DI で追加し、default は `createDefaultAuditMiddlewares()` が返す `[logging, validate, rate-limit]` の 3 本。logging は `DEFAULT_TOOL_CALL_LOG_PATH = "out/.state/tool-calls.jsonl"` に file sink で書き込む (acceptance criterion のパス)。middleware: [] で無効化も可能
- [x] テスト: 各 middleware が期待通りに wrap することを確認 — `tests/middleware/integration.test.ts` に 7 ケース (wrap order 2 + createAuditAgent DI 4 + factory constant 1)。**順序検証の要点**: invalid args ケースで `sleep spy の calls.length === 0` + handler 未呼び出しを assert → validate が rate-limit より外側にあることを決定論的に固定。valid args ケースでは logging の resultPreview に handler 戻り値が入り、invalid args ケースでは `[validate]` rejection が入ることで logging が最外側にあることも固定
- [x] Review (typecheck + test + `/code-review`) — 319 tests 全通過 (前 312 + 7 new)、typecheck エラーなし
