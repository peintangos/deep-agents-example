...
- [x] テスト: 関連性マッチングで正しい Skill が読み込まれることを確認 — `tests/skills-progressive-disclosure.test.ts` で minimal `createAgent` + `fakeModel` + `createSkillsMiddleware` + `CompositeBackend(/skills/ → FilesystemBackend)` を組み合わせ、10 ケースで source scoping / system prompt 注入 / frontmatter shape を決定論的に固定。このテスト実装時に `listSkillsFromBackend` が 1 階層走査制限を持つことが判明し、5 観測 subagent の `DEFAULT_<SUBAGENT>_SKILLS` を `/skills/audit/<aspect>/` → `/skills/audit/` に修正 (bug fix + 回帰ガード 1 ケース)。
- [x] Review (typecheck + test + `/code-review`) — 269 tests (+10 new - 1 revised) 全通過、`npm run typecheck` エラーなし
