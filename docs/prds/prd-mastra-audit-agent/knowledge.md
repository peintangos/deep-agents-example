...
- **Skill 配線の粒度は 3 階層で固定 (spec-007)**: (1) **メインエージェント** = `/skills/audit/` + `/skills/report/` の 2 ソース (オーケストレータとして全観点を俯瞰 + 最終レポート文体も知る必要がある)、(2) **5 観測サブエージェント** (license/security/maintenance/api-stability/community) = 各自 1 ソース `/skills/audit/<aspect>/` だけ (段階的開示の原則)、(3) **critic** = `/skills/audit/` 全体 (観点間の矛盾検出にはすべての判断基準を理解する必要があるため例外的に広い)。report 系は critic にも観測サブエージェントにも流れない — レポート生成は orchestration 層の責務で、サブエージェントは関与しないから。deepagents v1.9 では custom subagent はメインの skills を継承しないので、各 factory が自力で `skills: [...]` を返す必要がある (general-purpose だけが継承する)。`DEFAULT_<SUBAGENT>_SKILLS` 定数を各 factory で export し、テスト側で default 一致 + custom override の 2 アサーションを固定した。
