...
- **Validation middleware は zod の上に 2 段目として重ねる (spec-008)**: `tool()` に渡した zod schema は `z.string().min(1)` のような入口レベルのチェックしか担保しないので、GitHub の命名規則 (owner: 英数字 + ハイフン / trailing hyphen 禁止、repo: 先頭ドット禁止 / path traversal 禁止) のような **意味的な不正** は通ってしまう。middleware 層で regex + 個別条件を 2 段目として重ねる設計にした。GitHub owner の trailing hyphen は character class `[A-Za-z0-9-]` が許容するため、regex の後に `endsWith("-")` を別途チェックする load-bearing ステップが必須 (テストで固定済み)。**rejection 時は throw ではなく `ToolMessage(content: "[validate] ... rejected: ...", tool_call_id)` を返す** のが langchain の tool rejection パターン — LLM は次ターンで ToolMessage を読んで引数を修正できる。throw にすると agent 全体が reject 経路に入って監査が止まるので validation 用途には合わない。validator を `Record<toolName, (args) => ValidationResult>` で DI にすれば、新しいツール追加時に呼び出し側で `{...DEFAULT_TOOL_VALIDATORS, new_tool: ...}` のように merge できる。
- **langchain middleware の合成順序 (spec-008 最終確定)**: `node_modules/langchain/dist/agents/utils.js` L315 の `chainToolCallHandlers` は `result = handlers[last]` から始めて左向きに `composeTwo(handlers[i], result)` するので、**`middleware[0]` が outermost**、`middleware[N-1]` が innermost (handler に最も近い) となる。`createAuditAgent` のデフォルト middleware スタックは `[logging, validate, rate-limit]` の順で、この順序には 3 つの必然がある: (1) logging を最外にすると validation rejection も含めて "**全試行**" がログに残る (ToolMessage 返却は logging からは "success event" として見える)、(2) validate を中間に置くと不正引数は rate-limit の sleep 前に弾かれる、(3) rate-limit を最内に置くと sleep が handler の直前で発生し、**logging の `durationMs` にスロットル待ち時間が混入しない** (本物の tool 実行時間が測れる)。順序を入れ替えるとこれらのいずれかが崩れる。**決定論的に順序を検証する方法**: invalid args を渡して `sleep spy の calls.length === 0` と `handler 未呼び出し` を assert する — これで "validate が rate-limit より外" が固定される。valid args ケースでは logging の `resultPreview` に handler 戻り値が入り、invalid args ケースでは `[validate]` rejection が入ることで "logging が最外" も間接的に固定される。
- **`--target` の責務分離は「形式 CLI / 意味 middleware」(spec-009 step 1)**: `--target <owner/repo>` 実装時に regex を `src/cli.ts` と `src/middleware/validate.ts` の両方に持つか迷ったが、**形式チェック (ちょうど 1 つの `/`) は CLI 層**、**意味的バリデーション (GitHub 命名規則) は middleware 層の `validateGithubRepoArgs` を再利用** に分けた。利点: (1) regex が 1 箇所にまとまり drift しない、(2) CLI から投入されたケースと LLM が tool_call で投入したケースで拒否理由が一貫する、(3) `buildAuditPrompt` をユーザープロンプトの pure builder として切り出せば、監査手順は `AUDIT_SYSTEM_PROMPT` 側に完全に集約できる (ユーザープロンプトは「対象 OSS の identity を渡すだけ」の責務で drift しない)。`--invoke` (任意プロンプト) と `--target` (定型監査プロンプト) は同時指定を早期に弾く (両方とも invoker へプロンプトを渡す入口なので、優先順位を曖昧にしない)。テストは `runCli` 側の経路 8 + pure 関数 `parseTargetArg` 4 + `buildAuditPrompt` 2 = 14 ケース。`buildAuditPrompt` には「`/raw/license/result.json` などの orchestration 詳細を含まない」という drift ガードアサーションを入れておくと、将来ユーザープロンプトに手順を書き足したくなった時に警告が出る。
- **deepagents の state.files で `/raw/*/result.json` を返してもらう (spec-009 reporter 配線)**: `agent.invoke(...)` が返す state には `files?: Record<string, FileData>` があり、built-in `write_file` ツールが書き込んだファイルはここに現れる (`deepagents/dist/index.d.ts` L679/L739)。`CompositeBackend` は `/memories/` → StoreBackend / `/skills/` → FilesystemBackend だけを route するので、`/raw/` プレフィックスは **default 経路の StateBackend** に流れ、`state.files["/raw/license/result.json"]` で取れる (key は full path、prefix ストリップは backend 内部で完結する)。`FileData` は v1 (`content: string[]`) と v2 (`content: string | Uint8Array`) の union なので、`normalizeFileContent` で `Array.isArray → join`、`string → そのまま`、`Uint8Array → TextDecoder` の 3 分岐で 1 本の string に正規化する (`isFileDataV1` は top-level export されていないので自前判別)。reporter との配線は `extractAuditRawFromState(state)` → `{license, security, maintenance, apiStability, community, critic}` (全て `null` 許容) → `writeAuditReport({target, generatedAt, ...extracted}, path)` の順。raw JSON が不正な場合は **黙って null に落とさず path 付きで throw** する (agent 側の出力契約違反を即座に surface するため。"未取得" で黙って埋めると最終レポートの debug が地獄になる)。
- **JSDoc 内に `*/` を含む文字列を書くとコメントが早期クローズする (TS1161 / TS1434 トラップ)**: `/raw/*/result.json` のような glob パスや `*/foo` を **JSDoc コメント内 (`/** ... */`) に書く** と、埋め込まれた `*/` が JSDoc を途中でクローズしてしまい、以降のコメント行が code として parse されて TS1161/TS1434/TS1443 が雪崩のように出る。最初の error line (`scripts/run-audit.ts:88` のような) だけが原因で、残りは全部カスケード。**回避策**: JSDoc 内では `/raw/<aspect>/result.json` のように glob を angle bracket placeholder に置き換える、もしくは `/raw/${"*"}/result.json` のような escape テクニックは JSDoc では効かないので使わない。`*` 1 文字だけなら問題なく、危険なのは直後に `/` が続くケース。同じトラップは regex パターンを JSDoc に書いたとき (`/foo\*/bar/`) にも踏む可能性があるので、デバッグ時は最初の error line を見て直前に `*/` を含む文字列がないか最初に疑うこと。
- **raw 抽出は throw ではなく `{data, errors[]}` 方式にする (spec-009 初回ラン失敗から)**: Mastra 対象の初回 E2E で `critic` の JSON が 1 文字壊れ (`position 307` で "Expected ',' or '}'") ただけで `extractAuditRawFromState` が throw し、**5 観点の raw が揃っていても全レポートがゼロになる** 失敗モードを踏んだ。LLM の出力は一定確率で JSON syntax を壊すので、throw ベースの設計は本番で致命的。新しい契約: `readAspectRaw` / `readCriticFindings` は内部で try/catch し、parse / shape 失敗時は `data.<field>=null` にして `errors` 配列に `{path, error, rawString}` を push する。呼び出し側 (`realAuditRunner`) は errors があっても `writeAuditReport` を呼んで partial レポートを生成し、stderr に警告 + エラー件数 + dump path を出す。**`rawString` は元の string をそのまま持つ** ので、オフラインで `JSON.parse` を再実行してどこが壊れたか追える。`normalizeFileContent` は引き続き throw するが、per-field 関数が catch してエラー化するので throw は境界で吸収される。
- **`out/.state/last-run/` に raw を ダンプしてから抽出する (spec-009 debug loop)**: `realAuditRunner` の extraction 前に `dumpAuditRawFiles(state, LAST_RUN_DUMP_DIR)` を呼び、`state.files` の `/raw/*` プレフィックスを **そのまま** disk に保存する (normalize だけして parse せず)。agent の生出力 (壊れた JSON も含む) を失わず、次回ラン前に目視でき、オフラインで `extractAuditRawFromState` を再実行してデバッグできる。ダンプは毎ラン上書き (履歴は git で取る)。`.gitignore` は `out/` 全体を無視しているので追加設定不要。LLM 出力が不安定な現状では、**"agent が何を書いたか" を記録するのが reporter 層より優先度が高い**。
- **HITL の interrupt はメインエージェントの tool call にしか効かない可能性 (spec-009 初回ラン観測)**: Mastra の初回 E2E で `interruptOn: { fetch_github }` を設定していたが、5 subagent (license-analyzer 等) + main orchestrator の構成にもかかわらず **承認プロンプトが 1 回しか出なかった**。仮説: deepagents の `task` 委譲で起動される subagent の tool call は outer agent の `interruptOn` を継承しない (langgraph の checkpointer が別スレッドとして実行するため)。**裏を返すと、subagent 内の fetch_github は無制限に流れる** ので、rate-limit middleware だけが頼り。spec-008 で入れた `rate-limit` (min-interval 700ms) が load-bearing になっていることが実践的に裏付けられた。interruptOn を subagent にも伝播させたいなら (1) 各 subagent factory の `createDeepAgent` に個別に `interruptOn` を渡す、(2) 中央 HITL registry を共有する、のどちらかが必要だが、**監査が流れる速度と承認コストのトレードオフ** を考えると 1 回/ラン で十分な気もする (要判断)。
