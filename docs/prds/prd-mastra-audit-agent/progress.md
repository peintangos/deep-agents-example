...
| spec-006-human-in-the-loop | HITL 承認フロー統合 | in-progress | | `createAuditAgent()` に `checkpointer` (MemorySaver) と `interruptOn` を DI で注入可能にし、デフォルトで `fetch_github` + `query_osv` を承認対象に配線 (smoke tests 6 本追加)。CLI resume ハンドラ / HITL ログ / interrupt→resume E2E は残タスク |
