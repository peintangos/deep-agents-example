...
| spec-007-skills-integration | Skills 統合 | in-progress | | audit 5 SKILL.md (26 tests) + report/zenn-style SKILL.md (9 tests) + `createAuditAgent` への 3-way CompositeBackend ルーティング配線 (`/memories/`→Store, `/skills/`→Filesystem{virtualMode}, default→State) と `skillsRootDir`/`skills` の DI (smoke.test.ts spec-007 の 11 tests: exports 3 + 構成 4 + ルーティング 4)。残りは subagent 個別割当 / 段階的開示テスト |
