...
| spec-007-skills-integration | Skills 統合 | in-progress | | audit 5 SKILL.md (26 tests) + report/zenn-style SKILL.md (9 tests) + `createAuditAgent` 3-way CompositeBackend ルーティング配線 + `skillsRootDir`/`skills` DI (smoke 11 tests) + 6 subagent factory への `skills` DI (粒度 3 階層: 5 obs → 1 観点 / critic → audit 全体 / main → audit+report, 12 tests 追加)。残りは段階的開示テスト (fakeModel で SkillsMiddleware の prompt 注入を決定論的に検証) |
...
- Done: 6/9
- Current focus: spec-007-skills-integration (次)
