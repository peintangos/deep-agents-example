...
| spec-006-human-in-the-loop | HITL 承認フロー統合 | in-progress | | `createAuditAgent()` に `checkpointer` (MemorySaver) と `interruptOn` を DI で注入可能にし、`fetch_github` + `query_osv` を承認対象に配線 (smoke tests 6 本追加)。`src/hitl.ts` に pure core (detect / resolve / format / 2 presets、23 tests)、`scripts/run-audit.ts` に readline ベース consolePolicy + HITL ループを実装。HITL ログ (`/raw/hitl/log.jsonl`) / interrupt→resume E2E は残タスク |
