...
| spec-007-skills-integration | Skills 統合 | in-progress | | `skills/audit/{license,security,maintenance,api-stability,community}/SKILL.md` (26 tests) + `skills/report/zenn-style/SKILL.md` (9 tests, だ/である調 + 比較表 + `###` 3 段構成 + 総合判定テーブルテンプレ + critic findings 書き換えルール)。残りは createAuditAgent 配線 / subagent 個別割当 / 段階的開示テスト |
