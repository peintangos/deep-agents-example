...
| spec-009-e2e-mastra-audit | Mastra を対象にした E2E 実行と最終レポート生成 | in-progress | | `--target <owner/repo>` オプションを CLI に追加 (step 1/5 完了)。`src/cli.ts` の pure 関数 (`parseTargetArg` / `buildAuditPrompt`) と spec-008 `validateGithubRepoArgs` の再利用で 14 tests 追加、333 tests 全通過。残りは (2) Mastra 実 E2E / (3) 目視レビュー / (4) tool-calls.jsonl 統計抽出 / (5) 最終 review |

## Summary

- Done: 8/9
- Current focus: spec-009-e2e-mastra-audit (--target オプション実装済み、実 E2E 実行は API キー要)
