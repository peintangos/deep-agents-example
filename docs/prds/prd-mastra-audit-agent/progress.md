...
| spec-008-middleware | Middleware 統合 | in-progress | | `src/middleware/logging.ts` (ツール呼び出しロギング) を 4 層構成 (pure event / JSONL format / file sink / middleware) で実装し、13 tests で契約固定。残りは rate-limit / validate middleware + createAuditAgent への 3 本まとめて配線 |
