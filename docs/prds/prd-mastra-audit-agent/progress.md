...
| spec-008-middleware | Middleware 統合 | in-progress | | `src/middleware/logging.ts` (13 tests) + `src/middleware/rate-limit.ts` (min-interval 方式、`DEFAULT_GITHUB_MIN_INTERVAL_MS=700`、`now`/`sleep` DI で決定論化、11 tests)。残りは validate middleware + createAuditAgent 配線 + 順序テスト |
